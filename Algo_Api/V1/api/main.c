...
volatile uint32 ui = 0;//´®¿Ú½ÓÊÕÊý¾Ý³¤¶È!
uint8   rec_buf[256];
...
//ºìÍâ
uint8 FlagInfrared = 0;			// 0: idle 1: Ñ§Ï° 2£º½ÓÊÖ

//RF_RFM69H
RFM69H_DATA_Type TxBuf; 
uint8 FlagRF24GLearn = 0; // 0: idle 1: Ñ§Ï° 2£º½ÓÊÖ
...
typedef union //charÐÍÊý¾Ý×ªintÐÍÊý¾ÝÀà 
{  
	unsigned short  ue; 
	unsigned char 	 u[2]; 
}U16U8;
...
int tamain(void)
{
  ...
  //Command_Process();
    Init_DS18B20();
    timer2_init();                      //5ms ÖÐ¶Ï
	TIM3_NVIC_Configuration(); 		    //BSP_Delay ³õÊ¼»¯
   
    app_enroll_tick_hdl(isr_13us, 0);   //13usÔÚµ×²ãÅäÖÃµÄ£¬ÅäÖÃÍê³É¾Í¹Ø±ÕÁË
    Disable_SysTick();
...
//				U1_sendS((uint8*)InfraredSendCMD, sizeof(InfraredSendCMD));
		...
		{	
			U1_in();			//»ñÈ¡´®¿Ú·¢ËÍµÄSJÊý¾Ý!
			if(rec_buf[2] == ':')//½ÓÊÕµ½ÕýÈ·µÄ¿ØÖÆÊý¾Ý!
			{
//				//timer2_disable(); 
				switch(rec_buf[0])
				{
					 case 'L':
					 {
					 	switch(rec_buf[1])
						{
							case 'H': //ºìÍâÑ§Ï°
							{
								if((FlagInfrared==0) && InfraredEnterStudy())		 //ÐèÒª½âÎö£¬Èç¹ûÒÑ¾­ÔÚÑ§Ï°Ä£Ê½
								{
									FlagInfrared = 1;
...
//									U1_sendS((uint8*)ResFail, sizeof(ResFail));	//Ê§°Ü
				   ...
				   case 'D':
					   if(rec_buf[1]=='T') //ÎÂ¶È²É¼¯
					   {
					   		uint16 temp;
	
	//					    U1_sendS("»Ø¸´ÃüÁî", 5);
							while((temp = GetTemperature()) == 0x55);	   // ÕâÀï£¿ Èç¹ûÊÇ¸ºÖµ£¿  ÈçºÎÅÐ¶Ï²É¼¯Ê§°Ü
							U1_sendS("DT:", 3);		///¿ÉÒÔÓÅ»¯
							U1_sendS((uint8*)&temp, 2);	
							U1_sendS("<<", 2);	

					   }
					   else if(rec_buf[1]=='S')	//¼ì²âwifiÃüÁî
					   {
//					   		Check_wifi = 1;
//							Wifi_Command_Mode = 0;
							U1_sendS("DS<<",4);
					   }
					   else	if(rec_buf[1]=='M')	 //MAC µØÖ··¢ËÍ
					   {
							U1_sendS("DM:",3);
